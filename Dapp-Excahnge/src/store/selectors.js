...
export const myFilledOrdersSelector = createSelector(
	...
	(account, orders) => {
		...
		// console.log(Number(account) + " Account");
		// console.log(orders[0] ? Number(orders[0].user) : "");
		// console.log(
		// 	orders[0] ? Number(orders[0].user) === Number(account) : ""
		// );
		orders = orders.filter((o) => {
			return (
				Number(o.user) === Number(account) ||
				Number(o.userFill) === Number(account)
			);
		});
		//console.log(orders);
...
const decorateMyFilledOrder = (order, account) => {
	const myOrder = Number(order.user) === Number(account);
...
export const myOpenOrdersSelector = createSelector(
	...
	(account, orders) => {
		...
		orders = orders.filter((o) => Number(o.user) === Number(account));
		// Decorate orders - add display attributes
